...
void CandidateModelSet::filterRatesMPI(int finished_model) {
    ...
    for (model = 0; model <= finished_model; model++) {
        ...
        if (getScore(model) <= ok_score) {
            ...
            fprintf(stderr, "Process %d, Model %s, Rate %s\n", 
                MPIHelper::getInstance().getProcessID(),
                at(model).subst_name.c_str(), 
                rate_name.c_str()
            );
        }
    }
    ...
    auto processModel = [&](int model) {
        if (MPIHelper::getInstance().models->get_shared_memory(model) != DBL_MAX) {
            ...
            tree_string = at(model).evaluate(params, model_info, out_model_info,
                                            models_block, num_threads, brlen_type);
            fprintf(stderr, 
                "Model %d: %s evaluated in %f seconds by process %d\n", model + 1, 
            ...
            for (int i = 0; i < num_models; ++i) {
                if (i != model 
                    && MPIHelper::getInstance().models->get_shared_memory(i) != 0) {
                    best_score = min(
                        best_score, 
                        MPIHelper::getInstance().models->get_shared_memory(i)
                    );
                }
            }
            ...
            // for (auto i : out_model_info) {
            //     fprintf(stderr, "PROCESS %d - out_model_info CHECKPOINT %s: Key = %s, Value = %s\n",
            //         MPIHelper::getInstance().getProcessID(),
            //         at(model).getName().c_str(),
            //         i.first.c_str(),
            //         i.second.c_str()
            ...
            stringstream ostr;
            ostr.precision(10);
            ostr << at(model).subst_name << " " 
                << ((at(model).rate_name == "") ? "@" : at(model).rate_name) << " "
